import BagSvg from './BagSvg';
import KeySvg from './KeySvg';
import UserSvg from './UserSvg';
import CopySvg from './CopySvg';
import PlusSvg from './PlusSvg';
import FailSvg from './FailSvg';
import LogoSvg from './LogoSvg';
import EditSvg from './EditSvg';
import CardSvg from './CardSvg';
import GiftSvg from './GiftSvg';
import MailSvg from './MailSvg';
import TrackSvg from './TrackSvg';
import ApplePay from './ApplePay';
import CheckSvg from './CheckSvg';
import MinusSvg from './MinusSvg';
import HeartSvg from './HeartSvg';
import CameraSvg from './CameraSvg';
import PayPalSvg from './PayPalSvg';
import SearchSvg from './SearchSvg';
import BagBigSvg from './BagBigSvg';
import RatingSvg from './RatingSvg';
import EyeOffSvg from './EyeOffSvg';
import EyeOpenSvg from './EyeOpenSvg';
import GoogleSvg from './GoogleSvg';
import GoBackSvg from './GoBackSvg';
import MapPinSvg from './MapPinSvg';
import LogOutSvg from './LogOutSvg';
import BurgerSvg from './BurgerSvg';
import CommentSvg from './CommentSvg';
import FiltersSvg from './FiltersSvg';
import ViewAllSvg from './ViewAllSvg';
import PercentSvg from './PercentSvg';
import SuccessSvg from './SuccessSvg';
import TwitterSvg from './TwitterSvg';
import PayoneerSvg from './PayoneerSvg';
import CanceledSvg from './CanceledSvg';
import ShippingSvg from './ShippingSvg';
import ShoppingBag from './ShoppingBag';
import HeartBigSvg from './HeartBigSvg';
import FacebookSvg from './FacebookSvg';
import BagSmallSvg from './BagSmallSvg';
import CalendarSvg from './CalendarSvg';
import DeliveredSvg from './DeliveredSvg';
import RatingSelect from './RatingSelect';
import PlusInnerSvg from './PlusInnerSvg';
import EditSmallSvg from './EditSmallSvg';
import SortingBySvg from './SortingBySvg';
import PhoneCallSvg from './PhoneCallSvg';
import RightArrowSvg from './RightArrowSvg';
import CheckGreenSvg from './CheckGreenSvg';
import HeartInnerSvg from './HeartInnerSvg';
import MinusInnerSvg from './MinusInnerSvg';
import AddANewCardSvg from './AddANewCardSvg';
import PlusPaymentSvg from './PlusPaymentSvg';
import HomeAddressSvg from './HomeAddressSvg';
import CameraSmallSvg from './CameraSmallSvg';
import SearchSmallSvg from './SearchSmallSvg';
import MapPinSmallSvg from './MapPinSmallSvg';
import RepeatOrderSvg from './RepeatOrderSvg';
import LeaveAReviewSvg from './LeaveAReviewSvg';
import CategoryCheckSvg from './CategoryCheckSvg';
import MapPinAddressSvg from './MapPinAddressSvg';
import AddANewAddressSvg from './AddANewAddressSvg';
import RigthArrowBlackSvg from './RigthArrowBlackSvg';
import BriefcaseAddressSvg from './BriefcaseAddressSvg';

// Tabs
import HomeTabSvg from './tabs/HomeTabSvg';
import OrderTabSvg from './tabs/OrderTabSvg';
import ProfileTabSvg from './tabs/ProfileTabSvg';
import WishlistTabSvg from './tabs/WishlistTabSvg';
import CategoriesTabSvg from './tabs/CategoriesTabSvg';
import RatingStarSvg from './RatingStarSvg';
import PenSvg from './PenSvg';

export const svg = {
  BagSvg,
  KeySvg,
  PlusSvg,
  LogoSvg,
  EditSvg,
  CardSvg,
  GiftSvg,
  FailSvg,
  CopySvg,
  MailSvg,
  UserSvg,
  MinusSvg,
  CheckSvg,
  HeartSvg,
  ApplePay,
  TrackSvg,
  CameraSvg,
  BagBigSvg,
  EyeOffSvg,
  EyeOpenSvg,
  SearchSvg,
  GoBackSvg,
  RatingSvg,
  PenSvg,
  RatingStarSvg,
  GoogleSvg,
  LogOutSvg,
  MapPinSvg,
  BurgerSvg,
  PayPalSvg,
  PercentSvg,
  CommentSvg,
  ViewAllSvg,
  FiltersSvg,
  SuccessSvg,
  TwitterSvg,
  HomeTabSvg,
  PayoneerSvg,
  CanceledSvg,
  ShippingSvg,
  ShoppingBag,
  HeartBigSvg,
  FacebookSvg,
  BagSmallSvg,
  CalendarSvg,
  OrderTabSvg,
  DeliveredSvg,
  SortingBySvg,
  EditSmallSvg,
  PlusInnerSvg,
  PhoneCallSvg,
  RatingSelect,
  HeartInnerSvg,
  MinusInnerSvg,
  RightArrowSvg,
  ProfileTabSvg,
  CheckGreenSvg,
  CameraSmallSvg,
  HomeAddressSvg,
  PlusPaymentSvg,
  WishlistTabSvg,
  AddANewCardSvg,
  RepeatOrderSvg,
  SearchSmallSvg,
  MapPinSmallSvg,
  LeaveAReviewSvg,
  CategoryCheckSvg,
  CategoriesTabSvg,
  MapPinAddressSvg,
  AddANewAddressSvg,
  RigthArrowBlackSvg,
  BriefcaseAddressSvg
};
